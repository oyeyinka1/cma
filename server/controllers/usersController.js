...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    const verificationUrl = `<a href='http://localhost:3000/user/${newUser._id}:userId/verify/${verificationToken}/:token'>verify your email</a>`;

    const emailData = {
      email: newUser.email,
      subject: "Account created",
      text: `Hello ${newUser.firstName} ${newUser.firstName}, your account has been created. ${verificationUrl} to login.`,
    };
    //send email
    sendEmail(emailData);
